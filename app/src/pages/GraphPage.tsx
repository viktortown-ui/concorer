        ...
        {mode === 'map' && (
          <svg viewBox="0 0 820 420" className="graph-canvas" role="img" aria-label="Карта влияния">
            {mapEdges.map((edge) => {
              ...
              return <line key={`${edge.from}-${edge.to}`} x1={from.x} y1={from.y} x2={to.x} y2={to.y} stroke={edge.weight >= 0 ? '#43f3d0' : '#c084fc'} strokeWidth={Math.max(1, edge.absWeight * 6)} opacity={related ? 0.9 : 0.15} onClick={() => selectEdge(edge)} />
            })}
            {nodes.map((node) => {
